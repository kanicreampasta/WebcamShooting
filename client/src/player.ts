...
const collisionSphereMaterial = new THREE.MeshLambertMaterial({
  color: 0x00ff00,
  transparent: true,
  opacity: 0.1,
  wireframe: true,
...
export class Player {
  ...
  constructor(
    ...
    this.isOtherPlayer = isOtherPlayer ?? false;
  ...
  applyGun() {
    ...
    this.gun.point = new gAmmo.btVector3(
      position.x,
      position.y + 0.75,
      position.z
    );
    ...
    switch (this.gun.rate.type) {
      case "semi":
        if (lastTriggerState) {
          ...
          allowed =
            (time - this.lastShotTime) / 1000 >= this.gun.rate.minInterval;
        }
        break;
      case "auto":
